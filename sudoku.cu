...
__global__ void kernel() {
  ...
  while (__syncthreads_count(changed) != 0) {
    //continue propogating constraints
    for (int i=0; i < BOARD_DIM; i++) {
      printf("Hello from threadx: %d thready: %d\n", threadIdx.x, threadIdx.y);
      count ++;
      printf("count: %d\n", count);
    }
  }
...
void solve_boards(board_t* boards, size_t num_boards) {
  ...
  // copy the entire batch of boards to the GPU before running the kernel
  board_t* gpu_boards; // an array of 128 boards on GPU

  // Allocate space for the boards on the GPU
  if(cudaMalloc(&gpu_boards, sizeof(board) * BATCH_SIZE) != cudaSuccess) {
    fprintf(stderr, "Failed to allocate 128 boards on GPU\n");
    exit(2);
  }

  // Copy the cpu's x array to the gpu with cudaMemcpy
  if(cudaMemcpy(gpu_boards, boards, sizeof(board) * BATCH_SIZE, cudaMemcpyHostToDevice) != cudaSuccess) {
    fprintf(stderr, "Failed to copy boards to the GPU\n");
  }

  // Run the kernel
  dim3 threadsPerBlock(9,9);
  kernel<<<BATCH_SIZE,threadsPerBlock>>>();


  // copy the boards back off the GPU when the kernel is finished.
  ...
  if(cudaMemcpy(boards, gpu_boards, sizeof(board) * BATCH_SIZE, cudaMemcpyDeviceToHost) != cudaSuccess) {
    fprintf(stderr, "Failed to copy the boards from the GPU\n");
  }

  // Free the allocated boards on GPU
  cudaFree(gpu_boards);
}
